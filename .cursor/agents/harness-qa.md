...
**Scan targets:** Test directories, CI configuration, `cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/`,
test patterns, coverage reports.

**CI Ownership:** You are the ONLY subagent that runs `cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/`.
...
1. **Test coverage** — Run `cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/` and analyze results.
