...
```bash
cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/
```
...
1. Run the same test on the base branch:
   ```bash
   git stash && git checkout main && cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/ 2>&1; git checkout - && git stash pop
   ```
...
| 4 | `harness-qa` | `mode: code-eval`, diff range, `ci_command: cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/`, test directory listing |
...
- `memverse_domain: Be4Travel`
- Role-specific context listed above

**CI Ownership:** Only `harness-qa` runs `cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/`. Other roles
...
After auto-fixes, re-run `cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/` to verify no regressions.
...
3. Run `cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/` to verify
...
1. If ANY code changed after Step 3's test run (review fixes don't count),
   re-run `cd apps/api && source .venv/bin/activate && pytest && ruff check app/ tests/`. Paste fresh output — not a summary, the actual output.
